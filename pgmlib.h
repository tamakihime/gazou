...
/* 関数のプロトタイプ宣言 */
/* 階調画像を入力する関数 */
void load_image( int n, char name[] );  /* n:画像番号，name[]:ファイル名（""のときはキーボード入力）*/
/* 階調画像を出力する関数*/
void save_image( int n, char name[] );  /* n:画像番号，name[]:ファイル名（""のときはキーボード入力）*/
/* 階調画像をコピーする関数*/
void copy_image( int n1, int n2 );      /* 画像No.n1 を 画像No.n2にコピーする */
/* 階調画像を初期化する関数*/
...
/* 横幅，縦幅はそれぞれ width[n], height[n] に代入する */
{
    ...
    for (y=0;y<height[n];y++)
        fread(image[n][y], sizeof(unsigned char), width[n], fp);
    //    for (x=0;x<width[n];x++)
    //        image[n][y][x] = (unsigned char)fgetc( fp );
    fclose(fp);
    printf("%s(%dx%d)：画像は正常に読み込まれました．\n", file_name, width[n], height[n]);
}
...
/* 画像の横幅，縦幅はそれぞれ width[n], height[n] に代入されているものとする */
{
    ...
    /* 画像データの出力 */
    for (y=0;y<height[n];y++)
        fwrite(image[n][y], sizeof(unsigned char), width[n], fp);
    //    for (x=0;x<width[n];x++)
    //        fputc( image[n][y][x], fp );
    fclose(fp);
    printf("%s(%dx%d)：画像は正常に出力されました．\n", file_name, width[n], height[n]);
}
